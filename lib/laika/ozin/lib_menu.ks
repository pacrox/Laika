...
// 		list("Radio value:",			"radio",	{ parameter p is sv. if p <> sv toggle bool1. return bool1. }),
...
// >>>> WARNING <<<<
// THIS LIBRARY HAS BEEN MODIFIED TO MATCH LAIKA REQUIREMENTS.
...
if not (defined selectedLine) local selectedLine is 0. // << MODIFIED by PACROX
...
local refreshTypes is list("number","display","string","bool","radio").		// << MODIFIED by PACROX
...
function drawMarker {
	...
	else if lineType = "radio" {						// << MODIFIED by PACROX
		set suffixString to activeMenu[selectedLine][2]():tostring().
		print suffixString at (C3 + 1,startLine + selectedLine).
	}
...
local typesWithSuffix is list("number","radio","bool","menu","backmenu").	// << MODIFIED by PACROX
...
function inputs {
	if typingNumber { //currently recording numbers, so ignore all other commands
		...
		if ch = "w" or ch = inp:upcursorone {
			...
			if lineType = "menu" or lineType = "backmenu" or lineType = "bool" or lineType = "radio" or lineType = "action" adjust(0).						// << MODIFIED by PACROX
...
function updateLine {
	...
	if nonPaddedTypes:contains(valType) {
		...
		else if valType = "radio" {					// << MODIFIED by PACROX
			if val set valStr to " (+) ".
			else set valStr to   " ( ) ".
		}
