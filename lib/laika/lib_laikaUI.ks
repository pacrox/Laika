...
dPrint("12345", 0, 0).
LOCAL zp IS dPrint("12S35", -1, -1).
dPrint("12O45", -1, 0).
dPrint("12B45", 0, -1).

dPrint("12345", 1, 1).
LOCAL pp IS dPrint("12S35", -2, -2).
dPrint("12O45", -2, 1).
dPrint("12B45", 1, -2).

LOCAL icu IS dPrint("1CU5", "c", 0).
dPrint("1CB5", "c", -1).

LOCAL lin IS dPrint("--.", "l", -3).

FOR i IN RANGE(1,9) {
	dPrint("LBL" +i, "!" +(i/8-1/16), -4).
}
dPrint("CIAO", ">" +5/16, -5).
FOR i IN RANGE(3, 14) {
	dPrint(ROUND(1/i,3), "!" +(3/16), i).
}
...
dUpdate(lin, "-").
dUpdate(zp, "micano!").
dUpdate(pp, "riecchice").
dUpdate(icu, "Leeeellloooo").
}
...
FOR k IN RANGE(8)
	PRINT "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" AT(0,k).
...
LOCAL k IS 0.
UNTIL k > 8 {
	//PRINT k +" CIAO" AT(buttonCoords[k][0], buttonCoords[k][1]).
	drawSep(buttonCoords[k][0], buttonCoords[k][1], buttonCoords[k][2]).
	SET k TO k +1.
}
...
UNTIL k > 8 {
	//PRINT k +" CIAO" AT(buttonCoords[k][0], buttonCoords[k][1]).
	clearBox(buttonCoords[k][0], buttonCoords[k][1], buttonCoords[k][2], buttonCoords[k][3]).
	SET k TO k +1.
}

TERMINAL:INPUT:GETCHAR().

//FOR l IN ledCoords:KEYS 
//	PRINT led[1] AT(ledCoords[l][0], ledCoords[l][1]).
//
//TERMINAL:INPUT:GETCHAR().
...
UNTIL FALSE {
	...
	//FOR i IN RANGE(10) {
	//	WAIT 0.5.
	//	UISelect(UIcurrPage["button:sel"]+1).
	//}
